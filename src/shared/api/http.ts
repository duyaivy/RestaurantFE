...
import { getAccessTokenFromLocalStorage, normalizePath, removeTokensFromLocalStorage, setAccessTokenToLoacalStorage, setRefreshTokenToLoacalStorage } from "@/shared/lib/utils";
...
const request = async <Response>(
  ...
  try {
    payload = (rawText ? JSON.parse(rawText) : null) as Response;
  } catch (_parseError) {
    const isDev = process.env.NODE_ENV !== "production";
    const snippet = rawText.slice(0, 500);
    console.error("[http] Non-JSON response", {
      method,
      url,
      fullUrl,
      status: res.status,
    ...
    throw new HttpError({
      ...
      message:
        "Response is not valid JSON (check NEXT_PUBLIC_API_ENDPOINT or backend availability)",
    });
  }
  const data = {
    status: res.status,
  ...
  if (!res.ok) {
    if (res.status === ENTITY_ERROR_STATUS) {
      ...
      if (isClient) {
        ...
        // Nếu không có Authorization header → đây là lỗi credentials (sai pass/email)
        // không phải expired token → throw HttpError để route handler bắt
        if (!authHeader) {
  ...
  if (isClient) {
    ...
    if (
      ...
      setAccessTokenToLoacalStorage(accessToken);
      setRefreshTokenToLoacalStorage(refreshToken);
    } else if (
