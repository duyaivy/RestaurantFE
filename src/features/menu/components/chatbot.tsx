    ...
    <div className="px-4 mt-10">
      ...
      <div className="flex items-center gap-3 px-5 py-4 rounded-t-2xl border border-b-0 border-amber-600/40 bg-amber-950/20">
        ...
        <div className="text-left">
          <p className="text-stone-200 text-sm font-light tracking-wide">
            {t("menuCard.title")}
          </p>
          <p className="text-neutral-600 text-[11px] tracking-wider mt-0.5">
            {t("menuCard.subtitle")}
          </p>
        </div>
      </div>
        ...
        {CHATBOT_QUESTION_KEYS.map((questionKey, i) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span className="text-amber-700/40 text-[10px] font-mono">
                {String(i + 1).padStart(2, "0")}
              </span>
              <span className="text-xs text-neutral-400 tracking-wide group-hover:text-amber-300 transition-colors duration-200">
                {t(getChatbotQuestionTranslationKey(questionKey))}
              </span>
            </div>
            <span className="text-amber-500 opacity-0 group-hover:opacity-100 -translate-x-1 group-hover:translate-x-0 transition-all duration-200 text-sm">
              →
            </span>
          </button>
        ))}
